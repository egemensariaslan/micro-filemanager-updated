# Filemanager Plugin (updated for Micro 2.x)

> **Fork notice.** This is a maintenance fork of [NicolaiSoeborg/filemanager-plugin](https://github.com/NicolaiSoeborg/filemanager-plugin) (MIT, © 2017 Nicolai Søborg — see [`LICENSE`](./LICENSE)). The only behavioural change versus upstream 3.5.1 is a one-line fix in `preMousePress` so that clicking a tree entry works on Micro 2.x (the upstream still calls the Micro 1.4.x `GetMouseClickLocation` API and crashes with `filemanager:1092: attempt to call a non-function object`). See [`CHANGELOG.md`](./CHANGELOG.md) for details.
...
This fork is not in Micro's plugin registry. Install it manually:

```sh
git clone https://github.com/egemensariaslan/micro-filemanager-updated.git \
  ~/.config/micro/plug/filemanager
```

Then restart Micro and run `tree` (or bind `filemanager.toggle_tree`).

If you already have the upstream plugin installed via `plugin install filemanager`, remove it first (`rm -rf ~/.config/micro/plug/filemanager`) before cloning this fork into the same path.
