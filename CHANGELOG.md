...
- Tree-view mouse click crashed with `filemanager:1092: attempt to call a non-function object` on Micro 2.x. The `BufPane:GetMouseClickLocation(x, y)` method (Micro 1.4.x) was replaced in Micro 2.0.0 with `BufPane:LocFromVisual(buffer.Loc)`; `preMousePress` now uses the 2.x API.

### Note

- This release is published from the fork [egemensariaslan/micro-filemanager-updated](https://github.com/egemensariaslan/micro-filemanager-updated). Upstream is [NicolaiSoeborg/filemanager-plugin](https://github.com/NicolaiSoeborg/filemanager-plugin); all original copyright and the MIT license are preserved.
